...
// Load all product data files
const productDataFiles = import.meta.glob('../data/products/*.jsx', { eager: true });

export default function DynamicProductsPage() {
  const { productId } = useParams();
  const filePath = `../data/products/${productId}.jsx`;
  const module = productDataFiles[filePath];

  if (!module) return <div className="pt-24 text-white">Product not found.</div>;
